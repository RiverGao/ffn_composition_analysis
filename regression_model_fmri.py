...
for sec_id in range(1, 10):
    ...
    scores_arrays.append(section_start_label)  # n_perfix
    
    compo_scores = np.stack(scores_arrays, axis=0)  # (n_layer + 1, n_prefix)
    
    for pref_id in df_sec['pref_id']:
        x.append(compo_scores[:, pref_id])  # (n_layer + 1,)
...
# normalize x
x = zscore(x, nan_policy='omit')  # (n_word_total, n_layer), already an array
...
for v in range(n_vertex_in_mask):
    ...
    activations = []  # (n_word_total), fmri activations for this vertex
    ...
    for sec_id in range(1, 10):
        ...
        for offset in df_sec['sec_offset']:
            ...
            activation = fmri_vertex[timepoint]  # scalar
...
regression_betas = np.array(regression_betas)  # (n_vertex_in_mask, n_layer)
...
np.save(f'regression_results/model-fmri/scores/{model_version}_{subj_id}_scores.npy', regression_scores)
np.save(f'regression_results/model-fmri/betas/{model_version}_{subj_id}_betas.npy', regression_betas)
