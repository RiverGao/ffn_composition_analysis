...
regression_scores = [[], [], [], []]  # each element is a scalar
...
y_logfreq = zscore(y_logfreq)  # (n_word_total,)
y_bu = zscore(y_bu)
y_lc = zscore(y_lc)
y_td = zscore(y_td)
...
# plt.tight_layout()
# plt.savefig(f'figures/{model_version}_corrx_abs.png', dpi=120)
...
# do the regression for each layer
for lyr in range(n_layer):
    x_lyr = x[:, lyr].reshape(-1, 1)  # (n_word_total,)

    model.fit(x_lyr, y_logfreq)
    regression_scores[0].append(model.score(x_lyr, y_logfreq))

    model.fit(x_lyr, y_bu)
    regression_scores[1].append(model.score(x_lyr, y_bu))

    model.fit(x_lyr, y_lc)
    regression_scores[2].append(model.score(x_lyr, y_lc))

    model.fit(x_lyr, y_td)
    regression_scores[3].append(model.score(x_lyr, y_td))
...
# print(df_regression)
df_regression.to_csv(f'regression_results/model-control/{model_version}_results_control_single.csv', index=False)
...
target_labels = ['freq', 'bu', 'lc', 'td']
colors= ['#96C37D','#F8AC8C', '#2878B5', '#9AC9DB']
for i_tar, l_tar in enumerate(target_labels):
    offset = width * multiplier
    rects = ax.bar(x_loc + offset, regression_scores[i_tar], width, label=l_tar, color=colors[i_tar])
...
ax.set_title(f'Layerwise regression scores with the controlling variables (LLaMA2-{model_version})', fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'figures/{model_version}_model_control_single.png', dpi=120)